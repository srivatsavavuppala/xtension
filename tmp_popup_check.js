...
function getChatHistoryKey(owner, repo) {
  return `chatHistory_${owner}_${repo}`;
}

function saveChatMessage(owner, repo, message, sender) {
  if (!owner || !repo) return;
  
  const storageKey = getChatHistoryKey(owner, repo);
  chrome.storage.local.get({ [storageKey]: [] }, (result) => {
    let messages = result[storageKey] || [];
    messages.push({
      message: message,
    ...
    // Keep only last 100 messages per repo to avoid storage bloat
...
function loadChatHistory(owner, repo, chatMessages) {
  if (!owner || !repo || !chatMessages) return;
...
function clearChatHistory(owner, repo) {
  if (!owner || !repo) return;
  
  const storageKey = getChatHistoryKey(owner, repo);
  chrome.storage.local.set({ [storageKey]: [] }, () => {
    console.log(`[Chat] Cleared history for ${owner}/${repo}`);
  });
}
...
function createChatOverlay() {
  ...
  // Get repo info for better context
  chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
    chrome.tabs.sendMessage(tabs[0].id, {action: 'extractRepoInfo'}, (response) => {
      const repoInfo = response ? `${response.owner}/${response.repo}` : 'Repository';
      
      chatOverlay.innerHTML = `
        <div class="chat-header">
          <div style="display: flex; align-items: center; gap: 8px; flex: 1;">
            <span class="material-icons" style="color: var(--tab-active-color); font-size: 20px;">chat_bubble</span>
            <div style="flex: 1;">
              <div class="chat-header-title">${repoInfo}</div>
              <div style="font-size: 11px; color: var(--empty-desc); margin-top: 2px;">Ask anything about this code</div>
            </div>
          </div>
          <div style="display: flex; align-items: center; gap: 4px; flex-shrink: 0;">
            <button id="clear-chat-history-btn" title="Clear chat history" style="
      ...
      // Load chat history or show suggested questions on first load
      if (response && response.owner && response.repo) {
        // Store owner/repo in a data attribute for later use
        chatOverlay.dataset.owner = response.owner;
        chatOverlay.dataset.repo = response.repo;
        
        // Load previous chat history
        loadChatHistory(response.owner, response.repo, chatMessages);
        
        // Setup clear history button
        const clearBtn = document.getElementById('clear-chat-history-btn');
        if (clearBtn) {
          clearBtn.onclick = () => {
            clearChatHistory(response.owner, response.repo);
            chatMessages.innerHTML = '';
            showSuggestedQuestions(response.owner, response.repo);
            clearBtn.style.color = 'var(--empty-desc)';
          };
          
          // Highlight clear button on hover
          clearBtn.onmouseover = () => {
            clearBtn.style.background = 'rgba(200, 50, 50, 0.1)';
        ...
        // If no chat history exists, show suggested questions
        const storageKey = getChatHistoryKey(response.owner, response.repo);
        chrome.storage.local.get({ [storageKey]: [] }, (result) => {
          const messages = result[storageKey] || [];
      ...
      const createLoadingIndicator = function() {
        ...
        const interval = setInterval(() => {
          ...
          text.textContent = messages[currentMessage];
        }, 1500);
        
        // Store interval ID on the element for cleanup
        loaderDiv.dataset.intervalId = interval;
        
        return loaderDiv;
      };

      // Helper function to remove loader with interval cleanup
      const removeLoader = function(loaderDiv) {
        if (loaderDiv && loaderDiv.dataset.intervalId) {
          clearInterval(parseInt(loaderDiv.dataset.intervalId));
        }
        if (loaderDiv) {
          loaderDiv.remove();
        }
      ...
      const handleQuestion = function(question) {
        ...
        // Add new spinner loader
        const loaderDiv = createLoadingIndicator();
        chatMessages.appendChild(loaderDiv);
        chatMessages.scrollTop = chatMessages.scrollHeight;
        
        // Store old typingDiv reference for compatibility
        const typingDiv = loaderDiv;
        ...
        chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
          ...
          if (!currentUrl.includes('github.com')) {
            ...
            if (typingDiv.dataset.intervalId) {
              clearInterval(parseInt(typingDiv.dataset.intervalId));
            }
      ...
      const addReferences = function(references) {
        const refsDiv = document.createElement('div');
        refsDiv.className = 'message bot references-message';
        refsDiv.style.cssText = 'background: var(--modal-bg); border: 1px solid var(--modal-border); padding: 12px; border-radius: 12px;';
        
        let refsHtml = '<div style="font-size: 12px; font-weight: 600; margin-bottom: 8px; color: var(--modal-title); display: flex; align-items: center; gap: 6px;"><span class="material-icons" style="font-size: 16px;">link</span>Code References:</div>';
        
        references.slice(0, 3).forEach((ref) => {
          refsHtml += `
            <a href="${ref.url}" target="_blank" class="chat-ref-link" style="
              display: block;
              padding: 8px 10px;
              margin: 4px 0;
              background: var(--summary-bg);
            ...
            ">
              <div style="font-weight: 600; margin-bottom: 2px;">${ref.file_path}</div>
              <div style="font-size: 10px; opacity: 0.8;">Lines ${ref.start_line}-${ref.end_line}</div>
            </a>
          `;
        });
        
        if (references.length > 3) {
          refsHtml += `<div style="font-size: 10px; color: var(--empty-desc); margin-top: 8px; text-align: center;">+${references.length - 3} more references</div>`;
        }
...
modal.appendChild(header);
  ...
  function createAnalyzedHistoryItem(item) {
...
function createFavoriteHistoryItem(item) {
  const itemDiv = document.createElement('div');
  itemDiv.style.cssText = `
    background: var(--history-item-bg);
    margin: 12px 12px;
    ...
    <div style="margin-bottom: 8px;">
      <div style="display: flex; align-items: center; justify-content: space-between;">
        <div style="display: flex; align-items: center; gap: 10px; min-width: 0;">
          <img src="../icons/icon16.png" alt="Repo" style="width:18px; height:18px; margin-right: 6px; border-radius: 4px; flex-shrink: 0;">
          <a href="${item.url}" target="_blank" style="font-weight: 700; color: var(--modal-title); text-decoration: none; font-size: 13px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${item.owner}/${item.repo}</a>
        </div>
        <span style="color: var(--empty-desc); font-size: 11px; background: var(--summary-bg); padding: 3px 6px; border-radius: 4px; white-space: nowrap;">${formatTimestamp(item.timestamp)}</span>
      </div>
      <div style="color: var(--empty-desc); font-size: 13px; margin-top: 8px; line-height: 1.4;">${item.description || ''}</div>
      <div style="display: flex; gap: 8px; margin-top: 12px;">
        <button class="open-fav-btn" style="background: var(--tab-active-color); color: white; border: none; padding: 6px 10px; border-radius: 6px; font-size: 12px; cursor: pointer;">Open</button>
        <button class="remove-fav-btn" style="background: transparent; border: 1px solid var(--modal-border); color: var(--modal-title); padding: 6px 10px; border-radius: 6px; font-size: 12px; cursor: pointer;">Remove</button>
      </div>
    </div>
  `;

  const openBtn = itemDiv.querySelector('.open-fav-btn');
  ...
  itemDiv.addEventListener('click', () => {
    showMessage(item.description || item.repo, 'info');
    closeHistoryOverlay();
  });
